...
<?php else: ?>
    <?php foreach ($userFavorites as $favorite): ?>
        <a class="dropdown-item mt-0 px-0.5" href="#">
            ...
            <?php endif; ?>
            <form id="dashboard-favorite-delete-<?php echo $favorite["food_id"] ?>" action="<?php echo $globalpath ?>/includes/user_actions.inc.php" method="post" >
                <button class="dropdown-button" type="submit" name="favorite-delete"
                        value="<?php echo $favorite["food_id"] ?>" style="margin-left: 5px; float: right" onclick="deleteFavorite(this.value)">
