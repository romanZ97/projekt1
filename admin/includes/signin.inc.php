...
function checkLogin()
{
    if (!isset($_SESSION['user_id'])) {
        header("Location: views/singin.view.php");
        die();
    }
...
if(isset($_POST['login-submit'])) {
    ...
    if(empty($mail_username) || empty($pwd)){
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){
            header("Location: $globalpath/views/singin.view.php?error=sqlerror");
            ...
            if($row= mysqli_fetch_assoc($result)){
                ...
                if($pwdCheck == false){
                    header("Location: $globalpath/views/singin.view.php?error=wrongpwd");
